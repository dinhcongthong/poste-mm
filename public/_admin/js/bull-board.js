...
function loadDataTable() {
    $('.bulboa-datatables').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": {
            url: base_url + '/admin/bull-board/load-table-data',
            dataType: "json",
            type: 'POST'
        },
        "columns": [
            ...
            buttons: [{
                ...
                action: function (dialogRef) {
                    ...
                    BootstrapDialog.confirm({
                        ...
                        message: 'Do you really want to delete the bull-board ID: ' + articleId,
                        type: BootstrapDialog.TYPE_DANGER,
                        callback: function (result) {
                            if (result) {
                                $.ajax({
                                    url: base_url + '/admin/bull-board/delete',
                                    method: 'POST',
                                    data: {
                                        'id': articleId
                                    }
                                ...
                                .done(function (data) {
                                    ...
                                    if (data.result) {
                                        ...
                                        MyBootstrapShow('Delete bull-board Article', 'Delete Successfully', 'success');
                                    } else {
                                        MyBootstrapShow('Delete bull-boardArticle', data.error, 'danger');
                                    }
                                })
...
function changeStatus() {
    $('#table-view').on('click', '.change-status', function (e) {
        ...
        console.log(id);
        $.ajax({
            url: base_url + '/admin/bull-board/change-status',
            method: 'POST',
            data: {
                'id': id
            }
