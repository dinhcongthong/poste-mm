...
<div class="{{ !empty($article->target_student) ? 'text-primary' : 'span-color' }}">
    <div class="d-flex">
        <i class="fas fa-chart-line"></i>
        <span class="text d-flex align-items-center ml-2">対象者</span>
    </div>
@empty(!$article->target_student)
    <div class="feature-detail">
        <span class="text-dark d-flex align-items-center"> {!! $article->target_student !!}</span>
    </div>
@endempty
</div>

<div class="{{ !empty($article->fee) ? 'text-primary' : 'span-color' }}">
    <div class="d-flex">
        <i class="material-icons">attach_money</i>
        <span class="text d-flex align-items-center ml-2">授業料</span>
    </div>
@empty(!$article->fee)
    <div class="feature-detail">
        <span class="text-dark d-flex align-items-center"> {!! $article->fee !!}</span>
    </div>
@endempty
</div>
